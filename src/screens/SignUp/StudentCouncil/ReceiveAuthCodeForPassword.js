...
const ReceiveAuthCodeForPassword = ({ navigation, route }) => {
  ...
  const handleVerifyPassword = async () => {
    ...
    console.log('verifyCouncilPasswordFindEmailCode result', result);
  ...
  const handleResendAuthCode = async () => {
    ...
    console.log('resendCouncilPasswordFindEmailCode result', result);
    if (result.isSuccess) {
      // 기존 타이머 정리
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
