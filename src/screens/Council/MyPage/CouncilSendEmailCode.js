...
import { sendCouncilChangeEmailCode } from '../../../api/councilMyPage';
...
const CouncilSendEmailCode = ({ navigation }) => {
  ...
  const handleSendCode = async () => {
    ...
    setIsLoading(true);
    const result = await sendCouncilChangeEmailCode(email);
    setIsLoading(false);

    if (result.isSuccess) {
      navigation.navigate('CouncilVerfiyEmailCode', { email });
    } else {
      setEmailError(true);
    }
  };
